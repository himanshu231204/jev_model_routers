...
Releases are fully automated: **pushing a `v*` tag publishes the package to PyPI and creates a
GitHub Release.** Nothing is uploaded by hand and no API token is stored — PyPI trusts this
repository's `.github/workflows/publish.yml` through
[trusted publishing](https://docs.pypi.org/trusted-publishers/).

| | |
|---|---|
| PyPI project | [`jev-model-router`](https://pypi.org/project/jev-model-router/) |
| Workflow | [`.github/workflows/publish.yml`](.github/workflows/publish.yml) — runs on tags `v*` |
...
2. On GitHub, the workflow creates an environment named `pypi` on first use. Optionally add
...
8. **Watch the run** under *Actions → Publish to PyPI*. It:
